...
class Principal extends Component
{
    ...
    public function mount()
    {
        ...
        if (!Util::getExisteTiendaSeleccionadaLocalStorage()) {
          ...
          if (!Util::tienePermiso(Util::$OPCION_SALIDA)) {
            return redirect()->route('dashboard');
        }
    }
    ...
    {
        return view('livewire.salida.principal', ['data' => $this->modelarDatos()]);
    }
    ...
    public function modelarDatos()
    {
        ...
        $data = Salida::search($this->search, $this->filtrarPorEstado, $this->sucursal_empresas_id_seleccionado)->orderBy($this->sortField, $this->sortAsc ? 'asc' : 'desc')->simplePaginate($this->perPage);
    ...
    public function destruir()
    {
        ...
        try {
            foreach ($this->selectedItemTabla as $item) {
                $response = SalidaDetalle::where('salidas_id', '=', $item)->get();
            ...
            Salida::destroy($this->selectedItemTabla);
