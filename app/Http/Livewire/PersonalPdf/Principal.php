...
class Principal extends Component
{
    ...
    public function mount()
    {
        if (!Util::getExisteTiendaSeleccionadaLocalStorage()) {
          ...
          if (!Util::tienePermiso(Util::$OPCION_PERSONAL_PDF)) {
            return redirect()->route('dashboard');
        }
    }
    public function render()
    {
        return view('livewire.personal-pdf.principal', ['data' => $this->modelarDatos()]);
    }
    /**
    ...
    public function modelarDatos()
    {
        ...
        $data = PersonalPdf::search($this->search, $this->filtrarPorEstado, $this->sucursal_empresas_id_seleccionado)->orderBy($this->sortField, $this->sortAsc ? 'asc' : 'desc')->simplePaginate($this->perPage);
    ...
    public function destruir()
    {
        ...
        PersonalPdf::destroy($this->selectedPersonales);
    ...
    public function inactivarPersonales()
    {
        ...
        foreach ($this->selectedPersonales as $p) {
            $categoriaProducto = PersonalPdf::findOrFail($p);
