...
class Principal extends Component
{
    ...
    public function mount()
    {
        ...
        if (!Util::tienePermiso(Util::$OPCION_PERSONAL)) {
            return redirect()->route('dashboard');
        }
    }
    public function render()
    {
        return view('livewire.personal.principal', ['data' => $this->modelarDatos()]);
    }
    /**
    ...
    public function modelarDatos()
    {
        ...
        $data = Personal::search($this->search, $this->filtrarPorEstado, $this->sucursal_empresas_id_seleccionado)->orderBy($this->sortField, $this->sortAsc ? 'asc' : 'desc')->simplePaginate($this->perPage);
    ...
    public function destruir()
    {
        ...
        foreach ($this->selectedPersonales as $s) {
            $eliminar = true;
            $data = PersonalPdf::where('personals_id', '=', $s)->count();
            if ($data > 0) {
                $eliminar = false;
            }
            ...
            if ($eliminar == true) {
                ...
                Util::geterrordefine($this, "El Personal se encuentra en uso en otros recursos");
            }
        }


        if (count($selectEliminar) > 0) {
            Personal::destroy($selectEliminar);
    ...
    public function inactivarPersonales()
    {
        ...
        foreach ($this->selectedPersonales as $p) {
            $categoriaProducto = Personal::findOrFail($p);
